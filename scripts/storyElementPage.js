...
fabPanel?.addEventListener("click", (e) => {
  ...
  log(`🛠 FAB menu item selected: ${selectedTool}`);
...
function showFieldPrompt() {
  ...
  document.getElementById("fieldPromptModal").classList.remove("d-none");
}

function hideFieldPrompt() {
  document.getElementById("fieldPromptModal").classList.add("d-none");
}

document.getElementById("cancelFieldPrompt").addEventListener("click", hideFieldPrompt);
document.getElementById("closeFieldPrompt").addEventListener("click", hideFieldPrompt);

document.getElementById("confirmFieldPrompt").addEventListener("click", () => {
  const name = document.getElementById("fieldNameInput").value.trim();
  if (!name) return;

  hideFieldPrompt();

  // TODO: insert logic for the selectedTool
  log(`✅ Creating ${selectedTool} field with name: ${name}`);
  alert(`Would create a "${selectedTool}" field titled "${name}"`);
});
